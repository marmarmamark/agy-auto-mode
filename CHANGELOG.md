...
### Fixed
- Model pool is ordered by tier — flash-lite, then flash, then the Gemma reserves — with the newest version first inside each tier and the larger model first on a version tie. Previously the order was an artefact of list position: curated names outranked discovered ones, which put the Gemma reserves ahead of `gemini-3.6-flash` and `gemini-3.5-flash`. The ordering is applied to the cached pool on read, so a cache written by an older build or thinned by demotion still comes back in policy order.
- Model pool named `gemma-2-27b-it`, which 404s. This key serves `gemma-4-31b-it` and `gemma-4-26b-a4b-it`, so the entire high-capacity reserve tier was unreachable-by-name while being perfectly available. The installer probe now also discovers any listed model matching a wanted family, so a rename costs a model a slower position in the pool rather than its place in it.
- Daily classifier budget is derived from the reachable pool instead of a flat 1500. A key that can reach a 14,400/day Gemma reserve and one holding a single 500/day flash-lite were reported identically; on a pool of seven reachable models the real budget is 30,100.
